...
This scraper extracts job listings from the Michelin Workday ATS and imports them to peviitor.ro.

Target: https://michelinhr.wd3.myworkdayjobs.com/Michelin
...
1. **Start with brand** - We know the brand ("MICHELIN")
...
8. **Scrape new jobs** - Extract jobs from Workday cxs API (Romania)
...
company.js (validate company)
    ...
    │
    ▼ (if active)
scrape Workday cxs API (searchText "Romania")
    │
    ▼
...
- **Workday cxs API**: `https://michelinhr.wd3.myworkdayjobs.com/wday/cxs/michelinhr/Michelin/jobs` — POST JSON (`appliedFacets`, `limit`, `offset`, `searchText: "Romania"`)
