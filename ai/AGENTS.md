...
## Project
MICHELIN scraper for peviitor.ro (Node.js, ESM, Jest), derived from the EPAM template

## 🌱 This Repo Is a Derived Scraper
This repo is a **derived scraper** created from the [epam-systems-international-srl-nodejs-scraper](https://github.com/sebiboga/epam-systems-international-srl-nodejs-scraper) template.
...
- **Only the API parsing logic in `scraper/index.js`** (`fetchJobsPage`, `parseApiJobs`) is company-specific (Workday cxs API). The output shape (`mapToJobModel`, `transformJobsForSOLR`) must stay uniform across derived scrapers.
...
```bash
gh run view <RUN_ID> --repo sebiboga/michelin-romania-sa-nodejs-scraper --json status -q .status
```
...
**On every session:**
1. Check open GitHub issues: `gh issue list --repo sebiboga/michelin-romania-sa-nodejs-scraper --state open`
