...
| `scraper/job-validator.js` | Shared validation primitives - exports validateByHead(url), validateByContent(url, opts), validateByBrowser(url, opts), DEFAULT_EXPIRED_KEYWORDS. Used by `validate-jobs.js`, `tests/validate-michelin-jobs.js`, and the deep-validate workflow. |
...
| `tests/validate-michelin-jobs.js` | **Michelin-specific validator (used by CI).** Modes: `--head` (default), `--content`, `--browser` (Playwright). Called nightly by `automation-testing.yml` and manually via `job-deep-validate.yml`. Supports `--dry-run` and `--delete`. |
...
| `tests/e2e/scraper.test.js` | E2E tests - full pipeline with real Workday cxs API, ANAF, and Peviitor API |
...
- Full workflow: validate company (ANAF+CUIScan+CUIFirma+Peviitor) → scrape Workday cxs → transform → upsert → generate docs/jobs.md
